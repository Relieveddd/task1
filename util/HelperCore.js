...
class HelperCore {
  static jsonFuncQueryStr(jsonRouter) {
    ...
    if (
      jsonRouter !== null &&
      jsonRouter !== undefined &&
      jsonRouter.query !== null &&
      jsonRouter.query !== undefined
    ) {
  ...
  static queryStrFunc(paramName, jsonRouter) {
    ...
    if (
      jsonQueryStr !== null &&
      this.boolFuncJsonExistChk(jsonQueryStr, paramName)
    ) {
  ...
  static setValFuncQueryStr(paramNameIn, strParamValIn) {
    ...
    if (strHref.match(regExpNew)) {
      strReturnResult = strHref.replace(
        regExpNew,
        "$1" + paramNameIn + "=" + strParamValIn + "$2"
      );
    } else {
  ...
  static strFuncJsonFilter(strIn, boolActionId) {
    if (strIn === undefined || typeof strIn === "object") {
      ...
      if (boolActionId) {
        return strIn
          .replaceAll(":", "PlaceholderColon")
          .replaceAll(",", "PlaceholderComma")
          .replaceAll('"', "PlaceholderQuote");
      } else {
        return strIn
          .replaceAll("PlaceholderColon", ":")
          .replaceAll("PlaceholderComma", ",")
  ...
  static boolFuncJsonExistChk(jsonTarget, strKey) {
    if (
      ...
      jsonTarget === null ||
      Object.keys(jsonTarget).length === 0
    ) {
      return false;
    } else {
        ...
        srcElemCurrentMenuIn.addEventListener("click", function () {
          ...
          if (nodeListTargetMenu2 !== null) {
            nodeListTargetMenu2.forEach(function (srcElemTargetMenuIn) {
              if (
                srcElemTargetMenuIn.classList.contains("AuxiMarkCurrentTab18")
              ) {
          ...
          if (srcElemCurrentMenuIn.getAttribute("eg010") !== null) {
            //关闭所有同组元素.
            const nodeListTargetDetail = document.querySelectorAll(
              exprSelectorTargetDetailIn +
                '[eg010="' +
                srcElemCurrentMenuIn.getAttribute("eg010") +
                '"]'
            );
          ...
          const elemCurrentDetail = document.querySelector(
            exprSelectorTargetDetailIn +
              '[ei020="' +
              srcElemCurrentMenuIn.getAttribute("ei020") +
              '"]'
          );
    ...
    if (typeof objElemIn == "string") {
      ...
      if (nodeListSrc != null) {
        nodeListSrc.forEach(function (srcElemIn) {
          if (
            srcElemIn != null &&
            srcElemIn.classList.contains("AuxiMarkHide")
          ) {
    ...
    switch (strMethodInner) {
      ...
      case "DELETE":
        if (objReqParamBodyIn !== null) {
          const JS01reqParamInner =
            typeof objReqParamBodyIn === "string"
              ? objReqParamBodyIn
              : JSON.stringify(objReqParamBodyIn);
          const strQuery = new URLSearchParams(
            JSON.parse(JS01reqParamInner)
          ).toString();
...
class HelperUserAccount {
  static handlerLogin() {
    window.location.href =
      ConfigBiz.jsonFuncOption().API_OAuth.strURL +
      "/LV06UA010/UV04LP010?NumAI020=" +
      ConfigBiz.jsonFuncOption().AppInfo.NumAppId +
      "&LR01AT060=" +
      encodeURIComponent(ConfigBiz.URL_LocalFunc()) +
      "&FP03SC160=" +
      encodeURIComponent("/LayoutViewUserAccount/VerfyUserWebCmd");
  }
  static jsonFuncUserGroup(numUpdateModeIn) {
    const jsonStrUserGroup = sessionStorage.getItem("jsonArrayUserGroup");
    if (
      numUpdateModeIn === 3 ||
      (numUpdateModeIn === 2 && jsonStrUserGroup === null)
    ) {
      const promiseRespResult = HelperWebComm.reqFuncAccessToken(
        ConfigBiz.jsonFuncOption().API_Local.strURL +
          "/AttrUserAccount/QueryUserGroup",
        {},
        "get"
      ).then((strArrayRespResult) => {
        ...
        if (strArrayRespResult[0] === "NullAT060") {
          ...
          if (jsonRespResult.SC120 === "SuccGoal") {
            sessionStorage.setItem(
              "jsonArrayUserGroup",
              JSON.stringify(jsonRespResult.Payload.JsonArrayUserGroup)
            );
            jsonArrayUserGroupReturn =
              jsonRespResult.Payload.JsonArrayUserGroup;
          } else if (jsonRespResult.SC120 === "OverdueVT030") {
  ...
  static jsonFuncCurrentUserGroup() {
    return this.jsonFuncUserGroup(2).then((jsonArrayUserGroupIn) => {
      let jsonReturnResult = {
        jsonArrayUserGroup: null,
        jsonCurrentUserGroup: null,
      };
      if (jsonArrayUserGroupIn.length > 0) {
        jsonReturnResult.jsonArrayUserGroup = jsonArrayUserGroupIn;
        const strCurrentUserGroupNum = sessionStorage.getItem(
          "NumCurrentUserGroup"
        );
        if (strCurrentUserGroupNum === null) {
          jsonReturnResult.jsonCurrentUserGroup = jsonArrayUserGroupIn[0];
        } else {
          const jsonArrayCurrentUserGroup = jsonArrayUserGroupIn.filter(
            (jsonItem) => jsonItem.NG010 === Number(strCurrentUserGroupNum)
          );
          if (jsonArrayCurrentUserGroup.length > 0) {
            jsonReturnResult.jsonCurrentUserGroup =
              jsonArrayCurrentUserGroup[0];
          } else {
            jsonReturnResult.jsonCurrentUserGroup = jsonArrayUserGroupIn[0];
...
class DataModelDataGroup {
  static jsonFuncSystemMgt(numUpdateModeIn, numCurrentUserIn) {
    const jsonStrDataGroupInner = sessionStorage.getItem(
      "jsonDataGroupSystemMgt"
    );
    // const strCurrentUserGroupNum = window.localStorage.getItem("CG040");
    let jsonDataGroupInner =
      jsonStrDataGroupInner === null ? null : JSON.parse(jsonStrDataGroupInner);
    const StrKeyM0 = "NG01-" + numCurrentUserIn;
    if (
      numUpdateModeIn === 3 ||
      (numUpdateModeIn === 2 &&
        (jsonDataGroupInner === null ||
          !HelperCore.boolFuncJsonExistChk(jsonDataGroupInner, StrKeyM0)))
    ) {
      let promiseDataGroupInner = HelperWebComm.reqFuncAccessToken(
        ConfigBiz.jsonFuncOption().API_Local.strURL +
          "/DataAccessWithGroup/QueryDataGroup",
        { CurrentUserGroup: numCurrentUserIn },
        "get"
      ).then((strArrayRespResult) => {
        if (strArrayRespResult[0] === "SuccWebReq") {
          const jsonRespResult = JSON.parse(strArrayRespResult[1]);
          let statusCodeInner = HelperCore.strFuncCodeVerifyToken(
            "US040",
            jsonRespResult.SC120
          );
          if (statusCodeInner === "SuccGoal") {
            ...
            sessionStorage.setItem(
              "jsonDataGroupSystemMgt",
              JSON.stringify(jsonDataGroupInner)
            );
