...
onBeforeMount(() => {
  ...
  if (AuthCode.length > 0) {
    ...
    const PromiseReq = proxy.$HelperAxios
      .post(
        proxy.$ConfigBiz.jsonFuncOption().API_Local.strURL +
          "/UC03UV040/AR03AT060",
        jsonReqParam
      )
      .then((responseResult) => {
        ...
        const StrMsg = proxy.$HelperCore.strFuncJsonFilter(
          responseResult.Msg,
          false
        );
        if (responseResult.SC120 === "SuccGoal") {
          ...
          window.localStorage.setItem(
            "AccessToken",
            responseResult.Payload.AT060
          );
          // User Group
          let promiseUserGroup = proxy.$HelperUserAccount
            .jsonFuncUserGroup(2)
            .then((jsonArrayUserGroupIn) => {
              const URL_Return = proxy.$HelperCore.queryStrFunc(
                "LR01Return",
                proxy.router
              );
